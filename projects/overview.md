# Projects Overview

This section showcases my digital project experience, including internship projects and personal work.

## Project Categories

### Internship Projects
Projects completed during my internship at Latitude Innovation:
- [eAuction Car System](/projects/eauction-car-system) - Malaysia's biggest online auction
- [Bookshop System](/projects/bookshop-system) - Online bookshop for schools
- [Aircond Service System](/projects/aircond-service-system) - Aircond service system
- [Preschool Network Websites](/projects/preschool-network-websites) - Preschool website network
- [ESG Consultancy Website](/projects/consultancy-website) - ESG Consultancy website

### Academic Projects
- [DriveFlow](/projects/driveflow) - Driving school management (Final year project) 

<!-- ### Freelance/Personal Projects
- ... -->

<!-- ## Project Highlights

| Project | Type | Technologies |
|---------|------|--------------|
| eAuction Car System | Live Auction Platform | Laravel, Filament, Next.js, WebSocket |
| Bookshop System | E-commerce | WordPress, WooCommerce, PHP |
| DriveFlow | Management System | PHP, MySQL, Bootstrap |
| Preschool Network Websites | Multi-site Network | WordPress, Cloudflare |
| Consultancy Website | Business Website | WordPress |
| Aircond Service System | Aircond service system | Laravel, Filament, VitePress, Markdown | -->

## Skills Demonstrated

Through these projects, I've demonstrated proficiency in:
- **Full-stack development** (Frontend + Backend)
- **Database design** and management
- **API integration** and WebSocket implementation
- **Deployment** and server configuration
- **Technical documentation**

::: tip Project Selection
Click on any project to see detailed information including challenges faced, solutions implemented, and lessons learned.
:::
